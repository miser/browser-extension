...
(function() {
	...
	var MKImage = MKNoteWebclipper.Image = function(imgEl) {
		this.image = imgEl;
	}

	MKImage.prototype.download = function(callback, errFn, err404, noResponse) {
		var self = this;
	...
	MKImages.prototype.upload = function(successCallback, failCallback) {
		var self = this,
			images = self.images,
			note = self.note,
			option = self.option,
		...
		function saveImage(formData, successCallback, failCallback) {
			...
			HTTP.post(MKNoteWebclipper.options.baseUrl + "/attachment/savemany", events, formData)
		}
	}
